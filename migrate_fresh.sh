...
# Script pour réinitialiser et migrer complètement la base de données

echo "🗑️  Suppression de toutes les tables..."
mysql -u lyonpalme_user -p'VotreMotDePasseFort123!' lyonpalme << 'EOF'
...
if [ $? -eq 0 ]; then
    ...
    if [ $? -eq 0 ]; then
        echo ""
        echo "🎉 Base de données complètement initialisée!"
    fi
fi
